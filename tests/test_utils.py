...
class TestArgumentCollection:
    ...
    def test_collect_simple_arguments(self):
        ...
        mock_getter = MagicMock(side_effect=lambda name: {
            "file": "test.txt",
            "count": "5"
        }.get(name))
...
class TestEnvironmentPreparation:
    ...
    def test_prepare_flag_options(self):
        """플래그 옵션 환경변수"""
        options_metadata = [
        ...
        assert env["CLI_VERBOSE"] == "1"
        assert "CLI_DEBUG" not in env  # False 값은 설정하지 않음
