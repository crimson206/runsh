...
class DebugCommand(Command):
    ...
    arguments = [
        argument("script_name", "Name of the script to debug")
    ]
    ...
    options = [
        ...
        option("target-dir", None, "Target directory for exports", flag=False, default="debug_runsh"),
        ...
        option("show-runner-block", None, "Show generated SCRIPT-RUNNER block only", flag=True),
    ...
    def handle(self):
        ...
        try:
            # 임시 파일 생성해서 메타데이터 파싱
            temp_script_path = self._create_temp_original_file(script_content, script_name)
    ...
    def _show_metadata(self, metadata: dict):
        ...
        if args:
            ...
            for arg in args:
                ...
                self.line(f"    {arg['name']}: {arg.get('description', 'N/A')}{optional}{default}")
        ...
        if options:
            ...
            for opt in options:
                ...
                default = f" [default: {opt.get('default')}]" if opt.get('default') and not opt.get('flag') else ""
                self.line(f"    --{opt['name']}{short}: {opt.get('description', 'N/A')}{flag_info}{default}")
    ...
    def _validate_options(self, metadata: dict):
        ...
        for opt in options:
            ...
            if original_name != resolved_name:
                self.line(f"  <comment>Option conflict:</comment> '{original_name}' → '{resolved_name}'")
                conflicts_found = True
            
            if original_short != resolved_short:
                if resolved_short is None:
                    self.line(f"  <comment>Shortcut conflict:</comment> '-{original_short}' removed")
                else:
                    self.line(f"  <comment>Shortcut conflict:</comment> '-{original_short}' → '-{resolved_short}'")
                conflicts_found = True
